...
export const factoidCommands = Yargs()
  ...
  .command('*', false, {}, ({ message, ...argv }) => {
    Factoid.findOne({ command: argv._[0] })
      .exec()
      .then(result => {
        if (result) {
...
export function registerCommands() {
  ...
  commands.command(['factoid', 'factoids'], 'Manage factoids within slack.', yargs => {
    return yargs
      .showHelpOnFail(true)
      .demandCommand(1, '')
      .usage(
        `Usage: To use a actual factoid, I have to be in the channel and you can say \`!somefactoid\` for example.`
      )
      .epilogue(`Hint: Mention me (<@${userId}>) in a channel to invite me!`)
      .command(
        'create <command> [response..]',
        'Create factoids using snippets or text responses',
        {},
        async ({ command, response, message, ...argv }) => {
          ...
          if (message && message.files) {
            ...
            rtm.sendMessage(
              'There is no valid response or snippet available to create a factoid',
              message.channel
            );
          }
        }
