...
export function activate() {
  ...
  rtm.on('message', message => {
    ...
    if (channel.startsWith('D') || text.includes(`<@${rtm.activeUserId}>`)) {
      let command = text.replace(`<@${rtm.activeUserId}>`, '').trim();
  ...
  commands
    ...
    .command('slack', 'Fetch basic information about slack.', yargs => {
      return yargs
        ...
        .command(['whoami', 'me'], 'Your slack user id', {}, ({ message }) => {
          rtm.sendMessage(
            `Hi <@${message.user}>, your slack user id is: ${message.user}`,
            message.channel
          );
        })
        .command(['whereami', 'channel'], 'Current slack channel id', {}, ({ message }) => {
          rtm.sendMessage(
            `You're in <#${message.channel}>, this slack channel id is: ${message.channel}`,
            message.channel
          );
        });
    })
